...
##### 3.kv cache(kv缓存)   √
- 优化思路：当前推理每次预测下一个token时，需要重新计算qkv值，实际上kv的值只与当前输入的token有关，是固定不变的。因此可以缓存前面的kv值，每次预测只需要计算新的qkv值，减少计算量。
#### 测试结果
##### 1.生成1000个token的时间测试
| 名称 | 30次平均时间 |
|------|-------------|
...
| 裁剪到5000 | 20.54s |
| kv缓存+裁剪 | 3.12s |
- 详细数据可查看测试结果/create_1000_token.txt
- 从结果可以看出，kv缓存的方法有效，但裁剪反而增加了时间。初步怀疑是因为torch.topk()排序函数的计算成本超过了全量logits做一次softmax的计算成本，至于运行少量测试集推理的时间提升，可能是偶然现象，还需要多次实验证明。
